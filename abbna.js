...
function InitializeScript() {
  ...
  initialEntries.forEach((iterating_entry, index) => {
    ...
    const title = iterating_entry.querySelector(
      ":scope > .postTitle",
    ).textContent;
    // console.log(title);
    const postInfo =
      iterating_entry.querySelector(":scope > .postInfo").textContent;
    ...
    const author = iterating_entry.querySelector(
      ":scope > .postContent > .center > .center > a",
    ).textContent;
    ...
    const language = postInfo.slice(
      postInfo.indexOf("Language: ") + "Language: ".length,
      postInfo.indexOf("Keywords: "),
    );
    ...
    const postContent = iterating_entry.querySelector(
      ":scope > .postContent > p:last-child",
    ).textContent;
    const postedDate = postContent.slice(
      postContent.indexOf("Posted: ") + "Posted: ".length,
      postContent.indexOf("Format: "),
    );
    // console.log("Posted Date: " + postedDate)
    const format = postContent.slice(
      postContent.indexOf("Format: ") + "Format: ".length,
      postContent.indexOf(" / "),
    );
    // console.log("Format: " + format);
    const bitrate = postContent.slice(
      postContent.indexOf("Bitrate: ") + "Bitrate: ".length,
      postContent.indexOf(" Kbps"),
    );
    // console.log("Bitrate: " + bitrate);
    const size = postContent.slice(
      postContent.indexOf("File Size: ") + "File Size: ".length,
      -1,
    );
    // console.log("Size: " + size);
    const detailsPage = iterating_entry.querySelector(
      ":scope > .postMeta > .postLink > a",
    ).href;
